...
from webapp.api.user_transactions.UserTransactionsRepository import (
...
def add_user(
    login, password, name, lastname, person_type, user_type, role_id, verified=False
):
    user = User(
        login, password, name, lastname, person_type, user_type, role_id, verified
    )
    db.session.add(user)
    db.session.commit()
    return user
...
def create_accounts():
    ...
    repo.create(
        **{"user_id": 1, "account_number": "01503000000000000000", "account_type_id": 1}
    )

    repo.create(
        **{"user_id": 1, "account_number": "01503000000000000001", "account_type_id": 2}
    )

    repo.create(
        **{"user_id": 2, "account_number": "01503000000000000002", "account_type_id": 2}
    )

    repo.create(
        **{"user_id": 2, "account_number": "01503000000000000003", "account_type_id": 1}
    )


def add_currency():
    cs = [Currency("dolar", "$", "USD"), Currency("bolivar", "Bs.S", "BS.S")]
...
def populate_db():
    ...
    # add users
    add_user(
        "admin", "admin", "admin", "admin", "legal", "admin", 1, True
    )  # admin_role.id)
    add_user(
        "user", "user", "user", "user", "natural", "user", 2, True
    )  # user_role.id)
    # add account types
    add_accounts_types()
    create_accounts()

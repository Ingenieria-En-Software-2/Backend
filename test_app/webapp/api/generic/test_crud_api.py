...
class CrudApiTestCase(TestCase):
    ...
    def test_post(self):
        ...
        self.assertTrue(
            "Something went wrong creating resource" in str(context.exception)
        )
        ...
        with self.app.test_request_context("/?key=value", method="POST"):
            with self.app.test_request_context():
                ...
                with self.assertRaises(Exception) as context:
                    self.crud_api.post()
                self.assertEqual(
                    "Something went wrong creating resource", str(context.exception)
                )
    ...
    def test_put(self):
        ...
        self.assertTrue(
            "Something went wrong updating resource" in str(context.exception)
        )
    ...
    def test_put_without_id(self):
        with self.app.test_request_context("/?key=value", method="PUT"):
            with self.assertRaises(Exception) as context:
                self.crud_api.put()
            self.assertEqual(
                "Something went wrong updating resource", str(context.exception)
            )
    ...
    def test_delete(self):
        ...
        self.assertTrue(
            "Something went wrong deleting resource" in str(context.exception)
        )
    ...
    def test_delete_without_id(self):
        with self.app.test_request_context("/", method="DELETE"):
            ...
            self.assertEqual(
                "Something went wrong deleting resource", str(context.exception)
            )
